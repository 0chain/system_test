...
// Client represents Ethereum client, which uses Tenderly fork node to perform snapshots and revert changes using requests
// to EVM.
type Client struct {
...
func (c *Client) InitErc20Balance(tokenAddress, ethereumAddress string) error {
	resp, err := c.client.Call(context.Background(), "tenderly_setErc20Balance", tokenAddress, []string{ethereumAddress}, InitialBalance)
	if err != nil {
		return err
	}
	if resp.Error != nil {
