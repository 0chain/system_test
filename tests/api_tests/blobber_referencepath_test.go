...
func TestFileReferencePath(testSetup *testing.T) {
	...
	t.RunWithTimeout("Get file ref with allocation id, remote path should work", time.Minute*5, func(t *test.SystemTest) {
	...
	t.RunWithTimeout("Get file ref with invalid allocation id should fail", time.Minute*5, func(t *test.SystemTest) {
		sdkClient.StartSession(func() {
			apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
	...
	t.RunWithTimeout("Get file ref with invalid sign should fail", time.Minute*5, func(t *test.SystemTest) {
		sdkClient.StartSession(func() {
			apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
