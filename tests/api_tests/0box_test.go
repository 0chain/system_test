...
func Test0boxNftCollection(testSetup *testing.T) {
	...
	t.RunSequentially("4 List NFT collection id with zero nft collection id  should work", func(t *test.SystemTest) {
		...
		require.Equal(t, zboxNftCollectionIdList.NftCollectionCount, 2)
		fmt.Println(zboxNftCollectionIdList)
	})

	t.RunSequentially("4 Get NFT by collection with invalid collection id should give empty array", func(t *test.SystemTest) {
		teardown(t, firebaseToken.IdToken, zboxClient.DefaultPhoneNumber)
		csrfToken := createCsrfToken(t, zboxClient.DefaultPhoneNumber)
		description := "wallet created as part of " + t.Name()
		walletName := "wallet_name"
		...
		fmt.Println(zboxNftCollectionId)
		require.Equal(t, `{"error":"400: error getting nft collection"}`, response.String())
	})

	t.RunSequentially("4 Get NFT collection id with one nft collection id present should should work", func(t *test.SystemTest) {
		teardown(t, firebaseToken.IdToken, zboxClient.DefaultPhoneNumber)
		csrfToken := createCsrfToken(t, zboxClient.DefaultPhoneNumber)
		description := "wallet created as part of " + t.Name()
		walletName := "wallet_name"
		...
		require.Equal(t, zboxNftCollectionIdList.NftCollectionCount, 3)
	...
	t.RunSequentially("Get NFT collection by collection id with one nft present should work", func(t *test.SystemTest) {
		...
		//require.Equal(t, zboxNFTList.NftList)
	})
...
func Test0boxNft(testSetup *testing.T) {
	...
	t.RunSequentially("Get NFT collection by wallet id with one nft present should work", func(t *test.SystemTest) {
		csrfToken := createCsrfToken(t, zboxClient.DefaultPhoneNumber)
		wallets, _, _ := zboxClient.ListWalletKeys(t, firebaseToken.IdToken, csrfToken, zboxClient.DefaultPhoneNumber, "blimp") // This endpoint used instead of list wallet as list wallet doesn't return the required data
...
func Test0BoxWallet(testSetup *testing.T) {
	...
	t.RunSequentially("Delete Wallet should work with wallet present", func(t *test.SystemTest) {
		...
		// Get Wallet
		wallets, _, _ := zboxClient.ListWalletKeys(t, firebaseToken.IdToken, csrfToken, zboxClient.DefaultPhoneNumber, "blimp")
	...
	t.RunSequentially("Update Wallet with wallet present", func(t *test.SystemTest) {
		...
		// Get Wallet
		_, resp, _ := zboxClient.ListWalletKeys(t, firebaseToken.IdToken, csrfToken, zboxClient.DefaultPhoneNumber, "blimp")
...
func teardown(t *test.SystemTest, idToken, phoneNumber string) {
	...
	for _, app := range appType {
		wallets, _, _ := zboxClient.ListWalletKeys(t, idToken, csrfToken, phoneNumber, app) // This endpoint used instead of list wallet as list wallet doesn't return the required data
