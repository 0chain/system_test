...
func Test0BoxShortIO(testSetup *testing.T) {
	...
	t.RunSequentiallyWithTimeout("Create short link endpoint should respond", 2*time.Minute, func(t *test.SystemTest) {
		...
		// 200/201 = success, 400/403/500 = short.io not configured (acceptable on test chains)
		require.True(t, resp.StatusCode() != 404,
			"Short.io endpoint should exist. Got %d: %s", resp.StatusCode(), resp.String())
		t.Logf("CreateShortLink: %d — %s", resp.StatusCode(), resp.String())
	})

	t.RunSequentiallyWithTimeout("Create referral link endpoint should respond", 2*time.Minute, func(t *test.SystemTest) {
		headers := zboxClient.NewZboxHeadersWithCSRF(t, client.X_APP_BLIMP)
		Teardown(t, headers)
		err := Create0boxTestWallet(t, headers)
		require.NoError(t, err, "0box wallet setup")
		...
		require.NoError(t, err)
		require.True(t, resp.StatusCode() != 404,
			"Referral endpoint should exist. Got %d: %s", resp.StatusCode(), resp.String())
