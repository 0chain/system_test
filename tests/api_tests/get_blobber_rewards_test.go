...
func TestBlobberRewards(testSetup *testing.T) {
	...
	t.RunSequentiallyWithTimeout("Check if blobber, which already exists in allocation as additional parity shard can receive rewards, should work", time.Minute*10, func(t *test.SystemTest) {
	...
	t.RunSequentiallyWithTimeout("Check if the balance of the wallet has been changed without rewards being claimed, shouldn't work", time.Minute*10, func(t *test.SystemTest) {
		t.Skip("wait for reward fixes")
		apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
	...
	t.RunSequentiallyWithTimeout("Check if a new added blobber as additional parity shard to allocation can receive rewards, should work", time.Minute*10, func(t *test.SystemTest) {
		t.Skip("wait for reward fixes")
		apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
