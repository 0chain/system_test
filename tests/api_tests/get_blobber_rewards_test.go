...
func TestBlobberRewards(testSetup *testing.T) {
	...
	t.RunWithTimeout("Check if blobber, which already exists in allocation as additional parity shard can receive rewards, should work", time.Minute*10, func(t *test.SystemTest) {
	...
	t.RunWithTimeout("Check if the balance of the wallet has been changed without rewards being claimed, shouldn't work", time.Minute*10, func(t *test.SystemTest) {
		t.Skip("wait for reward fixes")
		sdkClient.StartSession(func() {
			apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
	...
	t.RunWithTimeout("Check if a new added blobber as additional parity shard to allocation can receive rewards, should work", time.Minute*10, func(t *test.SystemTest) {
		t.Skip("wait for reward fixes")
		sdkClient.StartSession(func() {
			apiClient.ExecuteFaucet(t, sdkWallet, client.TxSuccessfulStatus)
