...
func TestMultiDownload(testSetup *testing.T) {
	...
	t.RunSequentially("Multi download should work", func(t *test.SystemTest) {
		...
		blobberRequirements.Size = 1024 * 1000 * 4 * 20
		...
		for i := 0; i < 10; i++ {
			op := sdkClient.AddUploadOperation(t, allocationID, 1024*1000*4)
			ops = append(ops, op)
		}
		...
		for _, file := range files {
			...
			require.Equal(t, int64(1024*1000*4), sz.Size(), "file size mismatch expected %v actual %v", 1024*1000*4, sz.Size())
		}
	})
}
