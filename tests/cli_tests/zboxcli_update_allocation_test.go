...
func TestUpdateAllocation(testSetup *testing.T) {
	...
	t.Run("Update allocation with add restricted blobber should succeed", func(t *test.SystemTest) {
		// setup allocation and upload a file
		allocSize := int64(4096)
		fileSize := int64(1024)
		...
		wallet, err := getWallet(t, walletFile)
		require.Nil(t, err)
	...
	t.Run("Update allocation with replace blobber and add restricted blobber should succeed", func(t *test.SystemTest) {
		// setup allocation and upload a file
		allocSize := int64(4096)
		fileSize := int64(1024)
