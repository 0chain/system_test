...
func TestMinerFeesPayment(t *testing.T) {
	...
	t.Run("Send ZCN between wallets with Fee flag - Fee must be paid to miners", func(t *testing.T) {
		...
		allMinerDetails := getAllMinerDetails(t)
		...
		block := verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, escapedTestName(t))
		require.NotNil(t, block, "block containing payFees transaction not found")
		...
		areMinerFeesPaidCorrectly := verifyMinerFeesPayment(t, blockMiner, allMinerDetails, expectedMinerFee)
	...
	// block := verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "vestingpool")
	// 	blockMinerId := block.Block.MinerId
	// 	blockMiner := getMinersDetail(t, blockMinerId)
	...
	// 	block := verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "read_pool_lock")
	// 	blockMinerId := block.Block.MinerId
	// 	blockMiner := getMinersDetail(t, blockMinerId)
	...
	// 	block = verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "read_pool_unlock")
	// 	blockMinerId = block.Block.MinerId
	// 	blockMiner = getMinersDetail(t, blockMinerId)
	...
	// 	block := verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "write_pool_lock")
	// 	blockMinerId := block.Block.MinerId
	// 	blockMiner := getMinersDetail(t, blockMinerId)
	...
	// 	block = verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "write_pool_unlock")
	// 	blockMinerId = block.Block.MinerId
	// 	blockMiner = getMinersDetail(t, blockMinerId)
	...
	// 	block := verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "stake_pool_lock")
	// 	blockMinerId := block.Block.MinerId
	// 	blockMiner := getMinersDetail(t, blockMinerId)
	...
	// 	block = verifyPayFeesTransactionInBlock(t, startBlock, endBlock, wallet, "stake_pool_unlock")
	// 	blockMinerId = block.Block.MinerId
	// 	blockMiner = getMinersDetail(t, blockMinerId)
...
func verifyPayFeesTransactionInBlock(
	t *testing.T,
	...
	for round := startBlock.Round + 1; round <= endBlock.Round; round++ {
		...
		for i := range block.Block.Transactions {
			...
			if !blockFound && txn.ClientId == wallet.ClientID && strings.Contains(txn.TransactionData, escapedTestName(t)) {
				blockFound = true
			}
			if blockFound && strings.Contains(txn.TransactionData, "payFees") {
				payFeesFound = true
				break
			}
		}
...
func verifyMinerFeesPayment(t *testing.T, blockMiner *climodel.Node, allMinerDetails []*climodel.Node, expectedMinerFee int64) bool {
	for _, miner := range allMinerDetails {
		if miner.SimpleNode.ID == blockMiner.SimpleNode.ID {
			if blockMiner.StakePool.Reward-miner.StakePool.Reward == expectedMinerFee {
				return true
			}
		}
	}
...
func getAllMinerDetails(t *testing.T) (miners []*climodel.Node) {
	minerList := getMinersList(t)

	for _, miner := range minerList.Nodes {
		minerDetail := getMinersDetail(t, miner.ID)
		miners = append(miners, minerDetail)
	}
