...
func balanceMintsAndRewards(
	...
	_, _ = fmt.Println("\tround", endSnapshot[0].Round)
	...
	_, _ = fmt.Println("\ttotal rewards", totalTotalRewerds)
	_, _ = fmt.Println("\tunclaimed rewards", totalUnclaimedRewards)
	...
	_, _ = fmt.Println("\tcalculated total reward", (minerBlockReward+sharderBlockReward)*endSnapshot[0].Round)
	_, _ = fmt.Println()
}
