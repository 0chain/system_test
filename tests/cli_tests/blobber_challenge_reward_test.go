...
func stakeTokensToBlobbersAndValidators(t *test.SystemTest, blobbers []climodel.BlobberInfo, validators []climodel.Validator, configPath string, tokens []float64, numDelegates int) {
	...
	for i := 0; i < numDelegates; i++ {
		for _, blobber := range blobbers {
			...
			_, err := executeFaucetWithTokensForWallet(t, blobberDelegates[idx], configPath, tokens[tIdx])
			if err != nil {
				fmt.Println(err)
				return
			}
	...
	for i := 0; i < numDelegates; i++ {
		for _, validator := range validators {
			// add balance to delegate wallet
			_, err := executeFaucetWithTokensForWallet(t, validatorDelegates[idx], configPath, tokens[tIdx])
			if err != nil {
				fmt.Println(err)
				return
			}
