...
func TestBrokenScenarios(testSetup *testing.T) {
	...
	// jsonPrint challengePool
	printJson := func(message string, v interface{}) {
	...
	t.RunWithTimeout("Tokens should move from write pool balance to challenge pool acc. to expected upload cost", 10*time.Minute, func(t *test.SystemTest) {
		...
		fmt.Println("Challenge Pool Info: ", output)
		...
		printJson("Challenge Pool: ", challengePool)
		...
		fmt.Println("Minutes elapsed: ", minutesElapsed)

		actualExpectedUploadCostInZCN := intToZCN(int64(minutesElapsed * 1e9 / 1024))

		fmt.Println("Actual expected upload cost in ZCN: ", actualExpectedUploadCostInZCN)

		finalAllocation := getAllocation(t, allocationID)
		printJson("Final Allocation: ", finalAllocation)
		...
		fmt.Println("Total Change in Write Pool: ", totalChangeInWritePool)
		fmt.Println("Challenge Pool Balance: ", challengePool.Balance)
		fmt.Println("Actual Expected Upload Cost in ZCN: ", actualExpectedUploadCostInZCN)
	...
	t.RunWithTimeout("Upload from local webcam feed with a negative chunksize should fail", 5*time.Minute, func(t *test.SystemTest) {
		output, err := createWallet(t, configPath)
		require.Nil(t, err, "Failed to create wallet", strings.Join(output, "\n"))
	...
	// FIXME: add param validation
	t.RunWithTimeout("Upload from youtube feed with a negative chunksize should fail", 5*time.Minute, func(t *test.SystemTest) {
	...
	// FIXME: add param validation
	t.RunWithTimeout("Uploading youtube feed with negative delay should fail", 5*time.Minute, func(t *test.SystemTest) {
		output, err := createWallet(t, configPath)
		require.Nil(t, err, "Failed to create wallet", strings.Join(output, "\n"))
	...
	// FIXME: add param validation
	t.RunWithTimeout("Uploading local webcam feed with negative delay should fail", 5*time.Minute, func(t *test.SystemTest) {
		output, err := createWallet(t, configPath)
		require.Nil(t, err, "Failed to create wallet", strings.Join(output, "\n"))
