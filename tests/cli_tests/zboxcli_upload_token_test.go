...
func writePoolInfo(t *testing.T, cliConfigFilename string) ([]string, error) {
	...
	output, err := cliutils.RunCommand(t, "./zbox wp-info --json --silent --wallet "+escapedTestName(t)+"_wallet.json"+" --configDir ./config --config "+cliConfigFilename, 3, time.Second*2)
	return output, err
}
