...
// TODO: add methods to register and delete authorizers both from 0chain and ethereum networks
...
func TestBridgeBurn(testSetup *testing.T) {
	...
	t.Run("Burning ZCN tokens without ZCN tokens on balance, shouldn't work", func(t *test.SystemTest) {
	...
	t.Run("Get ZCN burn ticket, should work", func(t *test.SystemTest) {
		t.Skip("Skip till runners are updated to newer ubuntu")
		output, err := executeFaucetWithTokens(t, configPath, 2.0)
		require.Nil(t, err, "faucet execution failed", strings.Join(output, "\n"))
...
func registerAuthorizer(t *test.SystemTest, url string, retry bool) error {
	t.Logf("Register authorizer in 0chain network")
	cmd := fmt.Sprintf(
		"./zwallet auth-register --client_id %s"+
			"--client_key %s --service_charge %f"+
			"--url %s --silent "+
			"--configDir ./config --config %s --path %s",
		configPath,
		configDir,
...
func deleteAuthorizer(t *test.SystemTest, id string, retry bool) error {
	t.Logf("Delete authorizer in 0chain network")
	cmd := fmt.Sprintf(
		"./zwallet bridge-auth-delete --id %s --silent "+
			"--configDir ./config --config %s --path %s",
		id,
		configPath,
		configDir,
