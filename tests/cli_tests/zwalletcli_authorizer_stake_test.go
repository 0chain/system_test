...
func TestAuthorizerStake(testSetup *testing.T) {
	...
	t.Run("Staking tokens against invalid node id should fail", func(t *test.SystemTest) {
		...
		require.NotNil(t, err, "expected error when staking tokens against invalid authorizer but got output", strings.Join(output, "\n"))
		require.Len(t, output, 1)
		require.Equal(t, "stake_pool_lock_failed: can't get stake pool: get_stake_pool: authorizer not found or genesis authorizer used", output[0])
	})
	...
	t.Run("Locking unlocking tokens with invalid node id should fail", func(t *test.SystemTest) {
		createWallet(t)
