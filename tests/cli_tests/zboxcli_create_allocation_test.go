...
func TestCreateAllocation(testSetup *testing.T) {
	...
	t.Run("Create allocation with too large parity (Greater than the number of blobbers) Should Fail", func(t *test.SystemTest) {
		...
		require.True(t, len(output) > 0, "expected output length be at least 1")
		require.Regexp(t, regexp.MustCompile(`Error creating allocation: failed_get_allocation_blobbers: failed to get blobbers for allocation: {"code":"allocation_creation_failed","error":"allocation_creation_failed: Too many blobbers selected, max available \d*"}`), output[0], strings.Join(output, "\n"))
	})
