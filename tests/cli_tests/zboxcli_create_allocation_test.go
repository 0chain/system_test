...
func TestCreateAllocation(testSetup *testing.T) {
	...
	t.Run("Create allocation on restricted blobbers should pass with correct auth tickets", func(t *test.SystemTest) {
		...
		require.Nil(t, err, "could not get blobber1 auth ticket : "+err.Error())
		blobber2AuthTicket, err := getBlobberAuthTicket(t, blobber2.Id, blobber2.Url, wallet.ClientID)
		require.Nil(t, err, "could not get blobber2 auth ticket : "+err.Error())
