...
func Test0Box(testSetup *testing.T) {
	...
	t.Log(shared.ConfigData.BoxAccessToken)
	if shared.ConfigData.BoxAccessToken == "" || shared.ConfigData.BoxRefreshToken == "" || shared.ConfigData.BoxClientId == "" || shared.ConfigData.BoxClientSecret == "" {
		t.Skip("Box Credentials were missing")
	}
	t.SetSmokeTests("Should migrate existing Box folder and files successfully")

	t.RunSequentially("Should migrate existing 	box folder and files  successfully", func(t *test.SystemTest) {
		...
		output, _ := cli_utils.MigrateFromCloud(t, cli_utils.CreateParams(map[string]interface{}{
			"access-key":    shared.ConfigData.BoxAccessToken,
			"secret-key":    shared.ConfigData.BoxRefreshToken,
			"client-id":     shared.ConfigData.BoxClientId,
			"client-secret": shared.ConfigData.BoxClientSecret,
