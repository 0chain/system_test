...
func TestZs3serverFanoutTests(testSetup *testing.T) {
	...
	conn, connErr := net.DialTimeout("tcp", config.Server+":"+config.HostPort, 5*time.Second)
	if connErr != nil {
		testSetup.Skipf("ZS3 server not available at %s:%s, skipping test: %v", config.Server, config.HostPort, connErr)
	}
	conn.Close()
