...
func TestWritePoolLockUnlock(testSetup *testing.T) {
	...
	t.Run("Should not be able to unlock unexpired write tokens", func(t *test.SystemTest) {
		...
		require.True(t, len(output) > 0, "expected output length be at least 1")
		require.Equal(t, "Failed to unlock tokens in write pool: write_pool_unlock_failed: can't unlock until the allocation is finalized or canceled", output[0]) //nolint
	})
}
