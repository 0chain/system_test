...
func TestFileDelete(testSetup *testing.T) {
	...
	t.Parallel()
	t.Run("delete directory with multiple existing file in root directory should work", func(t *test.SystemTest) {
	...
	t.Run("delete root directory with No existing file in root directory should work", func(t *test.SystemTest) {
		allocationID := setupAllocation(t, configPath)
		createAllocationTestTeardown(t, allocationID)
